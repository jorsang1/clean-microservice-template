using System.Runtime.CompilerServices;
using CleanCompanyName.DDDMicroservice.Application.Common.Interfaces;
using CleanCompanyName.DDDMicroservice.Application.Products.Commands.UpdateProduct;
using CleanCompanyName.DDDMicroservice.Domain.Entities.Products;
using FluentValidation;
using Microsoft.Extensions.Logging;

[assembly: InternalsVisibleTo("DynamicProxyGenAssembly2, PublicKey=")]
namespace CleanCompanyName.DDDMicroservice.Application.UnitTests.Products.ExposedHandlers;

internal class UpdateProductCommandHandlerExposed : UpdateProductCommandHandler
{
    public UpdateProductCommandHandlerExposed(
        IProductRepository productRepository,
        IDateTime dateService,
        ILogger<UpdateProductCommandHandlerExposed> logger,
        IValidator<Product> validator)
        : base(
            productRepository,
            dateService,
            logger,
            validator)
    {
    }
}